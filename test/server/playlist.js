...
describe("Test /playlist api", function () {
    ...
    it("should get 401 without token", function* () {
        yield agentFactory(server).get(constant.urls.playlist).expect(401).end();
    });
    
    it("should get 400 on invalid file type", function* () {
        yield agent.get(constant.urls.playlist).query({ path: "Hanasaku_Iroha_The_Movie.mp4" }).expect(400).end();
    });

    it("should get 404 on invalid path", function* () {
        yield agent.get(constant.urls.playlist).query({ path: "not exist.m4a" }).expect(404).end();
    }); 
});